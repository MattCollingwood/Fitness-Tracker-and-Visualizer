...
class FitTrack(QWidget):
    ...
    def initUI(self):
        ...
        self.distance_box.setPlaceholderText("Enter distance swam")
        ...
        self.table.setHorizontalHeaderLabels(["Id", "Date", "Calories", "Distance (yards)", "Description"])
        self.table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch) #removes white space in table section on right side of table
    ...
    # Load Tables
    def load_table(self): #calls table whenever app runs
        self.table.setRowCount(0)
        query = QSqlQuery("SELECT * FROM fitness ORDER BY date DESC")
        row = 0
        while query.next(): #while another query is available
            ...
            self.table.setItem(row, 0, QTableWidgetItem(str(fit_id))) #loads the data
            self.table.setItem(row, 1, QTableWidgetItem(date)) #loads the data
            self.table.setItem(row, 2, QTableWidgetItem(str(calories))) #loads the data
            self.table.setItem(row, 3, QTableWidgetItem(str(distance))) #loads the data
            self.table.setItem(row, 4, QTableWidgetItem(description)) #loads the data
            row += 1
    ...
    def delete_workout(self):
        ...
        if selected_row == -1: #If the user has not selected a row
        ...
        if confirm == QMessageBox.No:
            return #If the user clicks no, juist close the box.
        ...
        self.load_table() #reloads table after delete
